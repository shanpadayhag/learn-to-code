...
- `&[T]` — a [slice](../../languages/rust.md#slice): a borrowed view of a run of `T`s. So
  ...
  `candidate_words: Vec<&str>` and `&candidate_words` hands this function exactly that
  view.
...
`fn longest_common_prefix(candidate_words: &[&str]) -> &str` and the compiler asks:
*the `&str` you're returning — is it borrowed from `candidate_words`, or from something
local?* It can't guess, so it errors. The `'a` answers that question: "from
`candidate_words`." That's the whole job of the name — it
...
word is *already sitting in memory* — returning `&first_word[..common_prefix_length]`
hands back a view of it with
...
### The `take_while` closure (an inline, unnamed function)
`.take_while(|(first_word_byte, current_word_byte)| first_word_byte == current_word_byte)`.
The `|...| ...` is a **closure**: a small function written right where it's used, with
no name.

**Read the shape:** `|parameters| expression`. The bit between the pipes is the parameter list;
the bit after is the body (and its value is what the closure returns). So the closure
takes one argument, immediately destructures it into `first_word_byte` and
`current_word_byte`, and returns whether they're equal.
...
pattern `(first_word_byte, current_word_byte)` splits the pair, so the first name holds the
first word's byte and the second holds the other's; comparing them is `true` while the letters
match and `false` at the first difference — which is exactly where `take_while` stops.
...
> a peek: `take_while` actually hands the closure a *reference* to each item, so both
> names are `&u8`; Rust's "match ergonomics" quietly makes the pattern bind through that
> reference, and `&u8 == &u8` compares fine. You don't have to think about it — but
> that's why no `*` appears.

## Already in the handbook
- `if candidate_words.is_empty() { return ""; }` then
  `let first_word = candidate_words[0];` — a plain guard-then-bind. A tidier spelling is
  [`let … else`](../../languages/rust.md#let-else)
  (`let Some(first_word) = candidate_words.first() else { return ""; };`), but that was
  only **stabilized in Rust
...
- `standard_input.lock().lines()` — read input **one line at a time**, so the program
  answers as soon as the last word is typed. (An earlier version used `read_to_string`,
  which blocks until stdin *closes* — fine when input is piped, but in a real terminal it
  just waits for `Ctrl-D` and looks like it hangs. Reading a fixed
  `input_word_count + 1` lines avoids that.)
- `(0..input_word_count).map(|_| input_lines.next().unwrap().unwrap()).collect()` — pull
  exactly `input_word_count` lines into a `Vec<String>` (`owned_input_words`). Each
  `.lines()` item is a `Result<String, _>`, hence the two
  `.unwrap()`s: one for "was there a line", one for "did the read succeed".
- `owned_input_words.iter().map(|word| word.trim()).collect()` — build a `Vec<&str>` of
  trimmed views **borrowing from** `owned_input_words`. That owned `Vec<String>` stays
  alive to the end of `main`, so
