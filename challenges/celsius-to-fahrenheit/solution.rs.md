...
## The pieces
- `io::stdin().read_line(&mut input_line)` — read one line of input into the `String`
  `input_line`. It needs `&mut input_line` because it *writes into* the string
  ([`&mut`](../../languages/rust.md#mut-ref)).
- `input_line.trim().parse().unwrap()` — [`.trim()`](../../languages/rust.md#trim) drops the trailing
...
- `let celsius_temperature: f32 = ...` — the `: f32` annotation is doing real work: it's what tells `.parse()`
...
- `9.0 / 5.0` — float literals. The `.0` is the difference between `1.8` (float division) and `1`
  ([integer division](../../languages/rust.md#int-division)). Because `celsius_temperature` is `f32`, these
