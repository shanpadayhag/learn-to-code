...
Every concept the problem write-ups lean on is explained once here, in plain
language with no assumed programming background, and linked from wherever it
appears. Use it as a study path: follow a term, then follow *its* related terms
...
| [Double Free](double-free.md) | Freeing the same memory twice — corrupts the allocator and is a dangerous, exploitable bug that ownership prevents. |
...
| [Palindrome](palindrome.md) | Something that reads the same forwards and backwards, like `"level"` or `121`. |
